...
**Related:**
- [linstor-volsync-resize-inode-corruption.md](../../troubleshooting/linstor-volsync-resize-inode-corruption.md) — ext4 `resize_inode` corruption from crash-consistent snapshots
- [linstor-vgmknodes-after-reboot.md](../../troubleshooting/linstor-vgmknodes-after-reboot.md) — udev silently dropping `/dev/linstor_vg/*` symlinks after node reboot
...
This document captures everything learned across 2026-04-10 through 2026-04-14 so the knowledge isn't lost.
...
### Phase 5 — vgmknodes-after-reboot, the second class of failure (2026-04-14)

The next morning, opi03 was auto-rebooted by kured at 03:38 UTC after a NixOS auto-upgrade built a new generation (kernel 6.18.21 → 6.18.22). The kernel upgrade itself was clean. The reboot was clean. The satellite reconnected fine. **And yet** every LINSTOR-backed VolSync replication started failing within hours, with symptoms that looked nothing like Phases 1–4: stuck mover pods, snapshot timeouts, false DRBD split-brain reports, ArgoCD sync immutability errors, missing cache PVCs.

After ~4 hours of misdirected debugging across DRBD (resyncs, replica deletes/recreates), ArgoCD (sync options, ignoreDifferences, server-side apply, stripped last-applied-configuration annotations), and VolSync (snapshot deletes, manual triggers, controller restart), the actual root cause was found at the LVM layer: **udev had silently failed to recreate `/dev/linstor_vg/<pvc-uuid>_00000` symlinks for several LVs on opi02 and opi03**. The LVs were intact in LVM metadata, the device-mapper devices existed in the kernel (`dmsetup ls` confirmed), but the path symlinks LINSTOR satellites need to read block sizes during snapshot coordination were gone.

The error LINSTOR surfaced was `(opi0X) No response generated by handler` on the `ChangedData` API call — a wonderfully unhelpful symptom for a missing filesystem path. The satellite logs had the actual signal (`Failed to get block size of /dev/linstor_vg/...`) buried in a sea of routine output. Nothing in the upstream documentation or community issues mentions this failure mode.

**Fix per affected satellite**: `sudo vgmknodes linstor_vg`. 10 seconds. Reports which symlinks were missing as it recreates them. Note that `vgmknodes` is **lazy** — the first pass on opi03 fixed 1 symlink; a second pass an hour later (after a different sync attempt) revealed 2 more that hadn't been visible to the first run. Treat one pass as necessary-but-not-sufficient.

We don't know what causes udev to drop these symlinks. Plausible contributors (none verified): memory pressure during boot on the small Orange Pi nodes, race between LINSTOR satellite container startup and udevd readiness, ARM64-specific udev/dm event handling, regression in kernel 6.18.x. The bug doesn't affect every LV on every reboot — it's a different subset each time, with no obvious pattern.

**Mitigation**: kured DaemonSet pinned to a non-existent `nodeSelector` cluster-wide so it schedules on zero nodes. NixOS `system.autoUpgrade.allowReboot = false` is already in place on every LINSTOR satellite, so the only way to reboot one of these nodes now is manually. The manual reboot procedure (one node at a time, vgmknodes after each, test snapshot before moving on) is documented in [linstor-vgmknodes-after-reboot.md](../../troubleshooting/linstor-vgmknodes-after-reboot.md).

**Generalizable lesson**: every LINSTOR incident this week traced back to a simple primitive failing silently, with downstream symptoms that pointed in misleading directions. Phase 1–3 was tune2fs without e2fsck (10-second fix, 24-hour discovery delay). Phase 4 was an accidental RWX provisioning hiding the corruption. Phase 5 was vgmknodes (10-second fix, 4-hour discovery delay). The storage layer itself is solid — DRBD never lost data, LVM never lost data — but the operational surface keeps producing weird failures that aren't visible until they cascade. That cost is now too high to keep absorbing reactively. See "Decision Log" below for the 2026-04-14 direction shift.
...
**2026-04-14**: Third LINSTOR incident in three days. opi03 was auto-rebooted by kured at 03:38 UTC after a clean NixOS auto-upgrade (kernel 6.18.21 → 6.18.22). On reboot, udev silently failed to recreate `/dev/linstor_vg/<pvc-uuid>_00000` symlinks for several LVs on opi02 and opi03 — a failure mode none of us had ever seen, with downstream symptoms (DRBD "split-brain," `(opiXX) No response generated by handler`, ArgoCD sync immutability, VolSync stuck movers, missing cache PVCs) that pointed in completely wrong directions. ~4 hours of misdirected debugging before the root cause was found. Fix per node: `vgmknodes linstor_vg`. 10 seconds. Note that `vgmknodes` is **lazy** — first pass on opi03 fixed 1 symlink, second pass an hour later fixed 2 more. Documented in [linstor-vgmknodes-after-reboot.md](../../troubleshooting/linstor-vgmknodes-after-reboot.md). See Phase 5 above for the full incident narrative.

This is the third class of failure (after `resize_inode`/`s_reserved_gdt_blocks` corruption from Phase 1–3 and the RWX/NFS chokepoint discovered in Phase 4). Each one took hours to discover for what turned out to be a 10-second fix. The pattern is the same every time: a simple primitive in the LINSTOR stack fails silently, downstream symptoms point in misleading directions, debugging chases the symptoms before reaching the root cause. The storage layer itself is solid — DRBD has not lost data, LVM has not lost data — but the operational surface keeps producing surprises.

**Direction shift**: The "accept the risk and react" posture is no longer sustainable. Three actions taken today:

1. **kured pinned to a non-existent `nodeSelector` cluster-wide** so the DaemonSet schedules on zero nodes. NixOS package upgrades still happen via `system.autoUpgrade` (which already has `allowReboot = false` on every LINSTOR satellite), but the automatic reboot orchestration is held. Any node reboot now requires manual intervention with the documented post-reboot procedure (one node at a time → `vgmknodes` → test snapshot → uncordon → next node). To re-enable kured, drop the `nodeSelector` block from `kubernetes/base/infra/system/kured/helm-values.yaml`.

2. **Begin evaluating OpenEBS Mayastor** as a LINSTOR replacement. We already run OpenEBS for local PVs, so the operator stack is familiar. Mayastor specifically also closes the `fsfreeze` gap from Phase 1–3 (it implements `FIFREEZE`/`FITHAW` from the CSI node plugin by default), so a successful migration would solve **two** of the three failure classes in one move. Ceph is explicitly out of scope. Migration plan TBD; pick a low-risk app for a proof of concept.

3. **Defer Priorities 2 and 3 from the 2026-04-13 entry** (upstream issue filing, VolSync copy-trigger automation). If we're moving off LINSTOR, those investments don't pay back. Priority 1 (monitoring/alerts for stuck movers and out-of-sync ReplicationSources) is still worth doing now since it covers Mayastor too — it's already done, see commit `b691928` and the `VolSyncOutOfSync` / `VolSyncMoverStuck` PrometheusRules.

Open questions specific to the Mayastor evaluation:
- Storage pool layout — Mayastor uses NVMe-over-Fabrics, what does the device topology look like on the existing opi/raccoon nodes?
- Replication factor — does Mayastor support 3-way like LINSTOR, and what's the failure-tolerance story?
- Snapshot semantics — confirm `fsfreeze` is on by default and not just a flag we'd need to remember to set.
- Migration path — VolSync should be able to mediate the move (restore from restic into a fresh Mayastor PVC, swap), but worth confirming on a test app first.
- Resource cost — is the per-node Mayastor agent footprint compatible with the small Orange Pi nodes? opi01 was at 14Gi/15Gi RAM as recently as this morning (Minecraft Java VM was the biggest culprit; cleared after).

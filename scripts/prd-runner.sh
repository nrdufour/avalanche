...
if ! (cd "$work_dir" && echo "$prd_content" | claude --print \
  --model sonnet \
...
if pr_output="$(cd "$work_dir" && fj pr create "feat(prd-${prd_id}): ${prd_title}" --base main --head "$prd_branch" --body "$pr_body" 2>&1)"; then

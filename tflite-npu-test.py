...
# Try multiple strategies to find libteflon.so
TEFLON_LIB = None

# Strategy 1: Check common Mesa paths
mesa_patterns = [
...
for pattern in mesa_patterns:
    ...
    if matches:
        # Prefer newer mesa versions (higher version numbers in path)
        TEFLON_LIB = sorted(matches, reverse=True)[0]
        break

# Strategy 2: Use nix-store to query
if not TEFLON_LIB:
    try:
        result = subprocess.run(
            ['nix-store', '-q', '--outputs', '/run/current-system'],
            capture_output=True, text=True, timeout=2
        )
        system_path = result.stdout.strip()
        if system_path:
            matches = glob.glob(f'{system_path}/sw/lib/libteflon.so')
            if matches:
                TEFLON_LIB = matches[0]
    except:
        pass
